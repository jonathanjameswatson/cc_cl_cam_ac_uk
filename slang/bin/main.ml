...
let i1 (file, e) =
  wrap file e (fun x -> Interp_1.string_of_value (Interp_1.interpret x)) "Interpreter 1"
;;
...
let i3 (file, e) =
  wrap file e (fun x -> Interp_3.string_of_value (Interp_3.interpret x)) "Interpreter 3"
;;
...
(* show compiled code *)
let i2cc (_file, e) =
  let _ = print_string (Interp_2.string_of_code (Interp_2.compile e)) in
  None
;;

let i3cc (_file, e) =
  let _ = print_string (Interp_3.string_of_code (Interp_3.compile e)) in
  None
;;

let i4cc (_file, e) =
  let _ = print_string (Jargon.string_of_listing (Jargon.compile e)) in
  None
;;
...
let rec run_interpreters file e expected_option = function
  ...
  | (use, interp, describe) :: rest ->
    ...
    then (
      ...
      | Some string_out ->
        let _ = show_output describe string_out in
        let _ = check_expected describe file string_out expected_option in
...
let () =
  process_inputs
    (if Option.run_tests
    then (
      try Tests.get_all_tests () with
      | Errors.Error s -> fatal_error "tests/" "Test.get_all_tests" s)
