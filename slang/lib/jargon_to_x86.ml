...
let emit_x86 e =
  ...
  let base_name = String.sub Option.infile 0 (String.length Option.infile - 6) in
  let out_chan = open_out (base_name ^ ".s") in
  let tab c = output_string out_chan ("\t" ^ c ^ "\n") in
  let cmd c comment =
    (*want comments to line up nicely! *)
    let l = String.length c in
    let tab_string = if l < 8 then "\t\t\t" else if l < 16 then "\t\t" else "\t" in
  ...
  let emitc = function
    ...
    | PUSH (STACK_HI _i) ->
      complain "Internal Error : Jargon code never explicitly pushes stack pointer"
    | PUSH (STACK_RA _i) ->
      complain "Internal Error : Jargon code never explicitly pushes return address"
    | PUSH (STACK_FP _i) ->
  ...
  in
  let defs, cl = comp [] e (* compile to Jargon code with Jargon.comp  *) in
  (* emit header *)
  tab ".text";
  tab ".extern alloc";
  tab ".extern read";
