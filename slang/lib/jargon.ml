...
let string_of_installed_code (code, size) =
  let rec aux k =
    if size = k
    then ""
    else string_of_int k ^ ": " ^ string_of_instruction code.(k) ^ "\n" ^ aux (k + 1)
  in
  aux 0
;;
...
let string_of_stack (sp, stack) =
  let rec aux carry j =
    ...
    then carry
      ...
      aux
        (string_of_int j ^ ": " ^ string_of_stack_item (Array.get stack j) ^ "\n" ^ carry)
        (j + 1)
  in
  aux "" 0
;;
...
let do_fst vm =
  ...
  | STACK_HI a ->
    ...
    | HEAP_HEADER (_, HT_PAIR) -> push (heap_to_stack_item vm.heap.(a + 1), vm1)
...
let do_snd vm =
  ...
  | STACK_HI a ->
    (match vm1.heap.(a) with
    | HEAP_HEADER (_, HT_PAIR) -> push (heap_to_stack_item vm.heap.(a + 2), vm1)
...
let compile e =
  ...
  let result = c (* body of program *) @ [ HALT ] (* stop the interpreter *) @ defs in
