...
let scan_line in_chan =
  ...
  | ex ->
    let _ =
      print_string ("Unexpected Scanf exception : " ^ Printexc.to_string ex ^ "\n")
    in
    None
;;
...
let get_all_tests () =
  let in_chan =
    try Scanf.Scanning.open_in manifest_file with
    | _ -> Errors.complain ("can't open file " ^ manifest_file)
  in
  get_all in_chan
;;
